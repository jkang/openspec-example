...
- **[业务基线治理]**: 包含 `high_level_journey.html` (用户旅程)、`service_blueprint.html` (Story 矩阵/服务蓝图)、`business_process.html` (流程图) 和 `domain_model.html` (Event-Storming 模型)。**位于 `docs/baseline/`**。
...
- `docs/baseline/`: **[业务基线]** 包含 Journey, Story Map, Process Flow 和 Event-Storming Domain Model。这是系统认知的核心沉淀。
