---
name: sync-specs
description: Sync delta specs from a change to main specs. Use when the user wants to update main specs with changes from a delta spec, without archiving the change. In this project it performs **Spec Sync only (change-level)**; Baseline Sync is executed separately at Epic level via /opsx:baseline/sync.
...
**Layered Sync 提示（本项目）**: 本技能只做 **Spec Sync（change 级）** —— 将 delta specs 合并入 `openspec/specs/`。**Baseline Sync**（回流 `docs/baseline/*.html`）不在本技能范围，由 Epic 全部 Story 归档后统一执行（`/opsx:baseline/sync`）。若本 change 是某个 Epic 的最后一个 Story，完成后提示执行 Baseline Sync。
...
6. **Baseline Sync (回流业务基线)**
   ...
   - **调用辅助技能**: 依次调用 `baseline/` 子目录下的辅助技能：
     ...
     - `process-flow`: 根据 `story.md` (L1/L2) 和 `specs/design.md` (L3) 更新 `business_process.html`。需提取 E2E 旅程节点和规则环节。
