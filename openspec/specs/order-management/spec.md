...
系统 SHALL 为订单记录渠道来源（R-ORDCH-001/002，Q7，story-miniprogram-order-channel）：
...
- **Priority**: P1
- **Rationale**: 客户电话询单时运营需一眼识别订单来源（微信 vs 网页）；Q7 已确认会话来源继承（防客户端伪造）+ 仅标识展示。
...
- **Priority**: P1
- **Rationale**: 运营一眼识别订单来源；MVP 保持极简（仅标识列，不做筛选）。
