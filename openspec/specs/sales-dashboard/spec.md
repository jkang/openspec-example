...
## Purpose
销售报表看板（Sales Dashboard）：为老板与运营提供**只读经营分析视图**——销售总览（销售额/订单量/客单价/优惠让利 4 指标 + 时间维度切换 + 销售趋势）与商品/分类排行（TOP10，见 ranking Story）。数据实时聚合自订单明细（`actualPaidCents` 口径），属于 `data-insights` Bounded Context。
...
- **WHEN** 运营请求 `GET /api/admin/dashboard/sales?dimension=week`
- **THEN** 返回状态码 200
...
系统 SHALL 返回所选时间区间内的优惠券效果：优惠让利总额（同销售总览口径）、使用优惠券订单数（区间内 `couponId` 非空的成交订单计数）、用券订单占比（用券订单数 ÷ 订单量 × 100，订单量=0 时占比=0）。
...
- **Priority**: P1
- **Rationale**: 运营定位爆款/滞销；排行口径与总览一致（快照价、状态集、时间归属），复用同一聚合语义（R-RANK-001/004）。
...
- **Priority**: P1
- **Rationale**: 运营评估类目健康度；占比用于类目结构判断（R-RANK-002/005）。
