...
新增 `user-admin` capability（**新增 taxonomy**，来自 idea 候选 Capabilities 第 3 条）：承载 **B 端用户管理**能力——用户列表（ID/昵称/手机号/订单数/注册日期/状态）、按手机号或昵称关键词检索、用户详情（基础信息 + 该用户订单聚合）、禁用/启用账户（`正常` ↔ `禁用` 状态流转），以及**运营角色权限门禁**（仅运营角色可见入口，客服无权限，手机号属敏感信息受权限约束）。story.md 业务规则 R-ADM-001~007 为本增量规格的行为契约。
...
系统 SHALL 对 `GET /api/admin/users*` 系列接口施加**运营角色**门禁（R-ADM-001）：仅持有 `role = 运营` 用户的会话凭证可访问；非运营角色（含客服、未登录）SHALL 返回 403 无权限，且 SHALL NOT 返回任何用户手机号等敏感信息（R-ADM-007）。B 端用户管理入口在前端仅运营角色可见。

- **Priority**: P0
- **Rationale**: research 访谈记录 2 信号：客服无权限访问全量用户资料（敏感信息保护）；权限门禁防止 B 端数据越权。
...
- **Bounded Context**: `User Context`（Story 1 新增 BC，本变更扩展承载 `user-admin` taxonomy——用户账户与 B 端生命周期管理边界）；只读消费 `Order Context`（订单聚合）。
- **Capability Taxonomy**: `user-admin`（**新增 taxonomy**，`domain_model.html` 现有映射无 B 端用户管理能力，需 Domain Model Sync 落位）。
- **Related Process Nodes**: `L1-06 履约与完成`（B 端按用户维度回查订单聚合）、`L2-01 进入结算`（B 端权限门禁前置）、`L3-02 执行资格校验`（用户状态作为登录/会话门禁事实来源，间接依赖）。
- **Related Service Blueprint Nodes**: `SB-STAGE-06`（订单归属沉淀后 B 端聚合回查）、`SB-CUSTOMER-06`（我的订单，B 端视角数据源）、`SB-BACKSTAGE-04`（复用归属查询；新增 `GET /api/admin/users*` 泳道节点，待 Baseline Sync 落位）。
- **Sync Assessment**: `domain_model.html` **Needs Sync: Yes**（`user-admin` 新增 taxonomy + User Context 扩展 + 用户状态机 正常↔禁用）；`service_blueprint.html` **Needs Sync: Yes**（新增 B 端用户管理泳道节点）。本 change 仅 Spec Sync，Baseline Sync 在 Epic 归档后统一执行。
