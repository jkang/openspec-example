# miniprogram-channel Specification (Delta)

> 增量文件：**新增 taxonomy**，本 change 建立主 spec（对齐 `openspec/specs/<capability-path>/spec.md` 结构）。治理归属：**新增 `Channel Context` BC**（`bc-channel → cap-miniprogram-channel` Governs 边，`docs/baseline/domain_model.html` 当前无此 capability——BC/节点/边在 Epic 归档后 Baseline Sync 时统一落位，本 delta 显式声明新增）。

## Purpose

承载 B 端小程序渠道配置能力：AppID / AppSecret（脱敏回显）/ 商户号（纯配置预留）/ 启用状态开关；写权限仅运营（扩展 R-ADM 门禁家族）；配置落盘即时生效；渠道停用后拒绝小程序新登录（CHANNEL_DISABLED）且既有会话不受影响。Phase 6 Epic 6.1 依赖链根（wechat-login / order-channel 的渠道底座）。
...
- **Priority**: P0
- **Rationale**: appsecret 是微信平台敏感凭证，严禁明文回显（research 访谈 4：周工信号"appsecret 不建议明文落盘回显，至少考虑脱敏"）；Q4 已确认脱敏方案。
...
- 状态来源为渠道配置 `enabled` 字段（落盘即时生效），由 wechat-auth 登录 API 在 Story 2 消费；本 Requirement 声明状态语义与门禁契约。
...
- **Bounded Context**: **新增 `Channel Context`**（`domain_model.html` 映射表当前无此 BC，需新增 `bc-channel` 节点与 `bc-channel → cap-miniprogram-channel` Governs 边——Baseline Sync 时落位）；复用 `Shared / Cross`（frontend-ui 横切支撑）
- **Capability Taxonomy**: **`miniprogram-channel`（新增 taxonomy）**（新增路径，kebab-case）
...
- `openspec-requirements/epics/epic-miniprogram-channel/prototypes/miniprogram-channel-admin.html`（已 HITL 确认）：「小程序渠道」配置表单 + 启用开关 + AppSecret 脱敏「重新配置」交互 + 运营/老板角色区分。
