...
拆分是需求漏斗的第 4 步。一个 Epic idea 太大时，先拆成一个 **storymap**（多个 Story 的映射），再逐个深化为 story。

## 产物

- `openspec-requirements/storymaps/<key>/storymap.md`
...
拆分**前**：
1. 读对应 `epic.md`，列出全部 **In Scope 项** 与 **Exit Criteria 项**（含 B 端承诺）。
2. 将每项作为"必须覆盖"清单。

拆分**后**：
3. 逐项对账：每个 Epic 承诺项必须有 ≥1 个 Story 承接。
4. 对账表写入 storymap 的「覆盖对账」章节：`Epic 承诺项 → 承接 Story → ✅/❌`。
5. **闭环校验**：存在 ❌ 未覆盖项时，必须**补拆 Story** 或**显式降级**（在 epic.md 说明理由并同步 Exit Criteria）；严禁留下"承诺但不交付"的 taxonomy。

## 步骤

1. **读上下文**：读已确认的 `ideas/<idea-key>.md` 与对应 `epic.md`。
