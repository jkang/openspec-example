...
- `openspec-requirements/stories/<key>/story.md`（已确认，业务面冻结交付物）
...
3. **合成 proposal.md**（在开发侧 change 内）：从 Story 的业务面字段映射：
   - **Why** ← Story 用户场景 / 使用动机
   - **What Changes** ← Story 范围（In/Out of Scope）
   - **Capabilities** ← Story 治理映射的 Capability Taxonomy（按 capability 列出，新增 taxonomy 保留标注）
   - **Process Alignment** ← 治理映射的 L1/L2/L3 节点
   - **Service Blueprint Alignment** ← 治理映射的 SB-STAGE-*/SB-<LANE>-* 节点
   - **Impact** ← 关联 idea 的架构影响分析
...
6. **回填需求侧状态**：在需求侧 `story.md` 的「交接状态」勾选"已交接"并记录 `changeName`；同步更新对应 `epic.md`（拆分状态 → 交付中）与 `storymap.md`。
7. **进入开发侧流程**：从 `proposal` 起步 → 按 capability 拆分生成行为规格 `specs/`（Story-specs，h3/h4 格式）→ design → tasks → apply → verify → sync → archive。

## 交接契约

- Story.md 是需求侧唯一冻结交付物（业务面）。
- 行为规格（Story-specs）由开发侧在 proposal 之后按 capability 拆分生成，需求侧不生成 specs/。
- 开发侧从 proposal 起步；若发现需求缺口，回关 `openspec-requirements` 的 `req-*` skill。
- 涉及 UI 的 Story 若无已确认原型，禁止交接。
