...
4. **Task Type & Strategy**: Classify the idea as an Epic, Story, Bug Fix, or Tech Debt, and determine the subsequent workflow strategy.
   - **Governance Mapping**: Must refer to `docs/baseline/domain_model.html` for **Impacted Bounded Contexts**, `docs/baseline/business_process.html` for impacted **L1/L2/L3 Process Nodes**, and `docs/baseline/service_blueprint.html` plus `.trae/skills/baseline/openspec-baseline-story-map/SERVICE_BLUEPRINT_STANDARD.md` for impacted **SB-STAGE-* / SB-<LANE>-*** nodes. Record these in `ideas/idea.md`.
...
- You MUST split it into multiple Stories.
- You MUST create an `openspec/epic-<key>.story-list.json` file to track the execution queue.
- Each story in the list should initially have `status: "planned"`.
- Do NOT proceed to create a change directory for the Epic itself; Epics are realized through multiple individual Story changes.
