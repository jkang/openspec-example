...
4. **Task Type & Strategy**: Classify the idea as an Epic, Story, Bug Fix, or Tech Debt.
   - **Governance Mapping**: Must refer to `docs/baseline/domain_model.html` to identify **Impacted Bounded Contexts**, `docs/baseline/business_process.html` to identify impacted **L1/L2/L3 Process Nodes**, and `docs/baseline/service_blueprint.html` plus `.trae/skills/baseline/openspec-baseline-story-map/SERVICE_BLUEPRINT_STANDARD.md` to identify impacted **SB-STAGE-* / SB-<LANE>-*** nodes. Record these in `ideas/idea.md`.
