...
> 关联原型（Epic 整体）: `epics/account-system/prototypes/account-login.html`（待生成并经用户 HITL 确认）
...
- 原型链接：`epics/account-system/prototypes/account-login.html`（Epic 整体原型，待 `/req:prototype` 生成并经用户 HITL 确认后链接）
