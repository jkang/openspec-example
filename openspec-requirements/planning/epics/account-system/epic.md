...
- **订单/优惠券真实归属**：**新下单订单** `userId` 绑定当前登录用户；「我的订单」按 `userId` 查询（仅返回本人订单）；优惠券发券实例 `userId` 非空归属真实用户。
...
- **存量 `user_dev` 数据迁移**：本阶段不做存量订单/优惠券的归属迁移（存量保留并视为无归属），仅新订单/新发券使用真实 userId（详见 idea 存量数据处置决策）。

## Exit Criteria (退出标准)

- ① 注册 E2E 通过：新买家注册成功后获得可用账户（状态 ACTIVE），可进入登录。
- ② 登录 E2E 通过：买家登录后跳转个人区，可下单，**新订单** `userId` 归属当前登录用户。
...
- **Impacted Bounded Contexts**: `account`（**新增**：账户/认证/会话/用户管理）、`cart`（Cart Context：用户会话归属）、`order`（Order Context：订单 `userId` 真实归属）、`coupon`（Coupon Context：发券 `userId` 非空）、`shared`（`frontend-ui` / `domain-model` 承接登录 UI 与归属语义）。
...
- **Impacted Process Nodes**: `L1-03 加购与准备`（用户会话前置、禁用账户登录准入）、`L1-04 下单结算`（未登录下单拦截）、`L1-06 履约与完成`（我的订单查询）、`L2-02 加载结算上下文`（身份上下文）、`L2-06 发起支付`（归属校验）。
- **Impacted Service Blueprint Nodes**: `SB-STAGE-01`~`SB-STAGE-06`、`SB-LANE-CUSTOMER`、`SB-LANE-OPS`、`SB-LANE-BACKSTAGE`、`SB-CUSTOMER-01`、`SB-CUSTOMER-04`、`SB-CUSTOMER-06`、`SB-OPS-06`、`SB-BACKSTAGE-02`、`SB-BACKSTAGE-04`。
- **Potential Domain Model Sync Triggers**: 新增 `account` BC；`User` 聚合（id/email/nickname/phone/status）；`ACTIVE/DISABLED` 状态机；`Order.userId` 真实化（新订单归属本人）；`Coupon.userId` 非空化。
- **Potential Service Blueprint Sync Triggers**: 新增账户阶段/泳道节点；`frontend-ui` 增加登录/注册/会话交互；`SB-BACKSTAGE-*` 增加认证与会话校验；`SB-OPS-06` 增加用户管理活动；capability mapping table 补 `account-management`/`user-session`/`user-admin` 三行。
...
- [ ] 已探索（idea 已确认）
- [ ] 拆分中（storymap 产出中）
- [x] **已拆分**（storymap 已确认，Story 覆盖对账完成；4 个 Story 已拆，正在产出 story.md 业务面交付物）
- [ ] 交付中（部分/全部 Story 已交接开发侧）
- [ ] 已完成（所有 Story 已交接且开发侧已归档）
