...
> ⚠️ UI 门禁：本 Story 涉及后台用户管理 UI，原型尚未生成（`req-prototype` 未产出），故「交接状态」不勾选待开发交接。原型经用户 HITL 确认后补充链接。

- 原型链接：`stories/account-system-04-admin-users/prototypes/admin-users.html`（待 `req-prototype` 生成并经用户 HITL 确认后链接）
