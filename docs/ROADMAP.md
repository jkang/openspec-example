...
- `Order` 已含 `totalCents / discountCents / actualPaidCents / couponId / items / userId / status` + **`channel`**（渠道来源，Epic 6.1）→ 销售分析与渠道维度字段完整。
...
- **范围**:
  - **Epic 6.1 `epic-miniprogram-channel` — 小程序渠道接入与账户打通** ✅ 已交付（2026-09-08）:
    - B 端小程序渠道配置（appid / appsecret 脱敏 / 商户号纯预留 / 启用状态；仅运营可写、老板只读，扩展 R-ADM 权限门禁；停用 → CHANNEL_DISABLED 拒绝新登录、既有会话照常）。
